  ...
  return (
    <div className={`w-full h-screen flex flex-col ${
      ...
      <div className={`border-b p-3 sm:p-4 flex items-center ${
        ...
        <button
          onClick={() => navigate(-1)}
          className={`mr-2 sm:mr-4 p-2 rounded-full transition-colors ${
            isDark 
      ...
      <div className={`border-b px-3 sm:px-4 py-2 ${
        ...
        <div className="flex items-center justify-between flex-wrap gap-2">
          <p className={`text-xs sm:text-sm ${
            ...
            <span className="font-medium">Loan:</span> ₹{loan.amount?.toLocaleString()} for {loan.purpose}
          </p>
          <div className={`flex items-center ${
      ...
      >
          ...
          Object.entries(groupedMessages).map(([date, dayMessages]) => (
            <div key={date}>
              ...
              <div className="text-center my-3 sm:my-4">
                <span className={`px-2.5 sm:px-3 py-1 rounded-full text-xs border ${
                  isDark 
              ...
              {dayMessages.map((message, index) => {
                ...
                return (
                  <div key={message._id} className={`mb-1 ${isLastFromSender ? 'mb-2 sm:mb-3' : ''}`}>
                    <div className={`flex ${isCurrentUser ? 'justify-end' : 'justify-start'}`}>
                      <div
                        className={`max-w-[85%] sm:max-w-xs lg:max-w-md px-3 sm:px-4 py-2 rounded-2xl shadow-sm ${
                          isCurrentUser
                            ? isDark 
        ...
        {otherUserTyping && (
          <div className="flex justify-start mb-3 sm:mb-4">
            <div className={`px-3 sm:px-4 py-2 rounded-2xl rounded-bl-md max-w-[85%] sm:max-w-xs shadow-sm ${
              isDark 
      ...
      <div className={`border-t p-3 sm:p-4 ${
        ...
        {showEmojiPicker && (
          <div className={`mb-2 sm:mb-3 p-2 sm:p-3 border rounded-lg ${
            isDark 
            ...
            <div className="grid grid-cols-6 sm:grid-cols-8 gap-1.5 sm:gap-2">
              {emojis.map((emoji, index) => (
                <button
                  ...
                  className={`text-lg sm:text-xl rounded p-1 transition-colors ${
                    isDark 
                      ? 'hover:bg-gray-600' 
                      : 'hover:bg-gray-200'
                  }`}
        ...
        <form onSubmit={handleSendMessage} className="flex items-end space-x-1.5 sm:space-x-2">
          <button
            ...
            className={`p-2 rounded-full transition-colors mb-1 flex-shrink-0 ${
              isDark 
                ? 'hover:bg-gray-700 text-gray-400' 
                : 'hover:bg-gray-100 text-gray-500'
            }`}
          ...
          <div className="flex-1 min-w-0">
            <textarea
              ...
              className={`w-full border rounded-lg px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none transition-colors ${
                isDark 
